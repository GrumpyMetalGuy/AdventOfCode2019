...
fn update_grid(grid: &mut Matrix<GridPoint>, directions: &Vec<&str>, wire_number: i32) -> () {
    ...
    for direction in directions {
        ...
        let mut length = length_str.parse::<usize>().unwrap();

        while length > 0 {
            let current_grid_point = grid.get(x as usize, y as usize).unwrap();

            let wires_visited = current_grid_point.wires_visited | wire_number;
            let mut wire_one_distance = current_grid_point.wire_one_distance;
            let mut wire_two_distance = current_grid_point.wire_two_distance;

            if wire_number == 1 {
                wire_one_distance = match wire_one_distance {
                    None => Some(cumulative_distance),
                    Some(already_travelled) => {
                        Some(i32::min(already_travelled, cumulative_distance))
                    }
                };
            } else {
                wire_two_distance = match wire_two_distance {
                    None => Some(cumulative_distance),
                    Some(already_travelled) => {
                        Some(i32::min(already_travelled, cumulative_distance))
                    }
...
fn main() {
    let mut grid: Matrix<GridPoint> = Matrix::new(ARRAY_SIZE as usize, ARRAY_SIZE as usize);

    let wire_one_directions = WIRE_ONE.split(",").into_iter().collect::<Vec<&str>>();
    let wire_two_directions = WIRE_TWO.split(",").into_iter().collect::<Vec<&str>>();

    update_grid(&mut grid, &wire_one_directions, 1);
    update_grid(&mut grid, &wire_two_directions, 2);

    let mut min_manhattan = ARRAY_SIZE + ARRAY_SIZE;
    ...
    for x in 0..ARRAY_SIZE {
        for y in 0..ARRAY_SIZE {
            ...
            if current_grid_point.wires_visited == 3 {
                min_manhattan = i32::min(min_manhattan, (x - CENTRE).abs() + (y - CENTRE).abs());
                min_time = i32::min(
                    min_time,
                    current_grid_point.wire_one_distance.unwrap()
                        + current_grid_point.wire_two_distance.unwrap(),
                );
            }
        }
    }

    println!("{}", min_manhattan);
    println!("{}", min_time);
}
