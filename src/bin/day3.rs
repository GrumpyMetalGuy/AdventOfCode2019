...
fn update_grid(directions: &Vec<&str>) -> HashMap<(i32, i32), i32> {
    ...
    for direction in directions {
        ...
        while length > 0 {
            ...
            let new_steps = match grid.get(&coords) {
                Some(&x) => x,
                None => cumulative_distance
            };

            grid.insert(coords, new_steps);
...
fn main() {
    ...
    let grid1 = update_grid(&wire_one_directions);
    let grid2 = update_grid(&wire_two_directions);

    let grid1_coords: HashSet<&(i32, i32)> = HashSet::from_iter(grid1.keys());
    let grid2_coords: HashSet<&(i32, i32)> = HashSet::from_iter(grid2.keys());

    let intersections = grid1_coords.intersection(&grid2_coords);

    let mut min_manhattan = 1000000;
    ...
    for intersection in intersections {
        ...
        min_manhattan = i32::min(min_manhattan, intersection.0.abs() + intersection.1.abs());
        min_steps = i32::min(min_steps, grid1.get(&intersection).unwrap() + grid2.get(&intersection).unwrap());
    }
